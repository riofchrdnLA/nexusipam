...
export const SubnetManager: React.FC<SubnetManagerProps> = ({ subnets, setSubnets, currentUser }) => {
  ...
  const handleCreateSubnet = async () => {
    if (!newSubnetData.name || !isValidCIDR(newSubnetData.cidr)) {
        alert("Invalid Data: Please check Name and CIDR format.");
        return;
    }
    
    setIsSaving(true);
    // Use generateEmptySubnet to ensure all IPs are AVAILABLE by default
    const records = generateEmptySubnet(newSubnetData.cidr);
    
    if (Object.keys(records).length === 0) {
         alert("Invalid CIDR or range too large/small for this demo.");
         setIsSaving(false);
         return;
    }

    const newSubnet: Subnet = {
      id: "temp", // ID generated by DB
    ...
    try {
        ...
        // Note: Realtime subscription in App.tsx will trigger data reload
        setShowAddModal(false);
        setNewSubnetData({ name: '', cidr: '', vlan: '' });
    } catch (error: any) {
        console.error("Failed to create subnet:", error);
        // Display specific error from Supabase/DB
        const msg = error.message || error.details || JSON.stringify(error);
        alert(`Failed to create subnet: ${msg}`);
    } finally {
        setIsSaving(false);
    }
  };
  ...
  const deleteSubnet = async (id: string) => {
      if(confirm('Are you sure you want to delete this subnet and ALL its IPs?')) {
          try {
